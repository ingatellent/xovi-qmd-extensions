...
Combine changeVerticalJump.qmd, gestureTapToPage.qmd, gestureToolbarHide.qmd, gestureToolbarShow.qmd, hidePageLabelsInFullscreen.qmd and hideShowToolbar.qmd to get a clean fullscreen view where you can tap in the corners to change pages. Or when viewing a portrait document in landscape mode: Tap in the corners to scroll a page down and then change page on the next tap.
...
Swipe out from the toolbar to hide the toolbar.

## gestureToolbarShow.qmd
Swipe in from the edge to show the toolbar - for horizontal toolbar, the swipe only works at the show toolbar button (in order not to interfere with native gestures)
...
**Must** be installed **together with** *gestureTapToPage.qmd* and must be installed **instead of** *enableShortcuts.qmd*. It then adds the following keyboard shortcuts
- Left/right arrow to change page
...
## hideShowToolbar.qmd
Hide the button for showing the toolbar. Should definitely only be used with gestureToolbarShow.qmd or a similar activation of a gesture to show the toolbar (if not you will be unable to get the toolbar back until you restart the device)
